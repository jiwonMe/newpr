...
describe("executeStack", () => {
	test("creates commit chain with correct parent links", async () => {
		...
		expect(commit0?.branch_name).toMatch(/^newpr-stack\/pr-42\/feature-branch\/00-feature-auth-[a-z0-9]{6}$/);
		expect(commit1?.branch_name).toMatch(/^newpr-stack\/pr-42\/feature-branch\/01-feature-ui-[a-z0-9]{6}$/);
